...
void Update(RenderWindow& window) {
    ...
    Vector2f moveVec = Vector2f(0.f, direction * paddleSpeed * dt);
    Vector2f newPos = (paddles[1].getPosition() + moveVec);

    if (!(newPos.y + paddleSize.y / 2.f > gameHeight || newPos.y - paddleSize.y / 2.f < 0)) {
        paddles[1].move(moveVec);
    }
